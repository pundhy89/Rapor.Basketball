...
const replacement = `              <input 
              ...
              <div className="flex flex-wrap items-center gap-4 mt-1">
                ...
                {localSettings.academy.appBgVideoUrl ? (
                  <div className="h-12 w-20 bg-slate-800 rounded border border-white/20 dark:border-white/20 flex items-center justify-center text-xs font-bold">VIDEO</div>
                ) : null}
...
content = content.replace(/<input[\s\S]*?placeholder="URL Gambar Background Utama \(Opsional\)"[\s\S]*?className="text-xs file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:hover:bg-blue-900\/30 dark:file:bg-blue-900\/20 dark:file:text-blue-300 dark:text-slate-300"\s*\/>/g, replacement);
