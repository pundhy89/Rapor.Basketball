...
const newHeader = `      {/* Banner */}
      <div className="relative w-full z-0 overflow-hidden shadow-sm">
        {academy.headerBgUrl ? (
          <div 
      ...
      {/* Top Header Sticky */}
      <header className="sticky top-0 z-10 bg-transparent dark:bg-transparent backdrop-blur-md border-b border-white/20 dark:border-white/20 shadow-[inset_0_1px_1px_rgba(255,255,255,0.4),0_8px_32px_0_rgba(31,38,135,0.07)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_8px_32px_0_rgba(0,0,0,0.2)] p-4 transition-colors duration-500 relative overflow-hidden -mt-[1px]">
...
const headerRegex = /\{\/\* Top Header & Banner \*\/\}[\s\S]*?<\/header>\s*<\/div>/;
content = content.replace(headerRegex, newHeader);

// Also fix the main content area margin
content = content.replace(/<main className="flex-1 w-full max-w-3xl mx-auto p-4 flex flex-col pb-24 relative z-10 -mt-16 sm:-mt-24">/g, '<main className="flex-1 w-full max-w-3xl mx-auto p-4 flex flex-col pb-24 relative z-10">');
